...
menu_vars() {
	...
	for i in 0 1 2 3 4 5 6 7; do eval "N${i}=\"\$BL(${i})\$NC\""; done
	for i in E C R; do eval "L${i}=\"\$BL(${i})\$NC\""; done
    SS_FILE="/jffs/scripts/services-start"; PORT="$GR$SSH_PORT$NC"
...
node_auth() {
    ...
    elif [ "$RETRY" = "1" ]; then node_choice="2"
    else
...
sys_log() { logger -p user.info -t "Auto_SSH_Key" "$1"; }
